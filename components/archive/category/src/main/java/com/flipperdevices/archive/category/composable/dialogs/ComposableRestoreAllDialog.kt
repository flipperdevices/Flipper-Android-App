...
@Composable
internal fun ComposableRestoreAllDialog(
    onAction: () -> Unit,
    onCancel: () -> Unit
) {
    val dialogModel = remember(onCancel, onAction) {
        FlipperMultiChoiceDialogModel.Builder()
            .setTitle(R.string.dialog_restore_all_title)
            .setDescription(R.string.dialog_restore_all_desc)
            .setOnDismissRequest(onCancel)
            .addButton(R.string.dialog_restore_all_confirm, onAction, isActive = true)
