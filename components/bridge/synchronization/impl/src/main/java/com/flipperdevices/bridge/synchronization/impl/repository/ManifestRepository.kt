...
import com.flipperdevices.bridge.synchronization.impl.model.toHashMap
...
class ManifestRepository {
    ...
    suspend fun compareWithManifest(keys: List<KeyWithHash>): List<KeyDiff> {
        ...
        val manifestKeys = manifestFile.keys.toHashMap()

        for (key in keys) {
            val keyInManifest = manifestKeys.remove(key.keyPath.path)
